...
class MoteurCalcul
{
     ...
     */
    public function calculer(Demande $demande, Vehicule $vehicule, ?float $margeTaux = null, ?Devis $devis = null): array
    {
        ...
        $postes = collect();
        if ($devis) {
            $postes = $devis->postes()->orderBy('date')->orderBy('ordre')->get();
        }
        ...
        // Cache de l'itinéraire par couple origine-destination/profil (REQ-T-06).
        if ($itineraire->source !== 'estimation') {
            $cacheKey = 'itineraire:'.md5(implode('>', array_map(fn ($p) => round($p['lat'], 4).','.round($p['lng'], 4), $points)).':'.$vehicule->id.':'.($itineraire->source));
            Cache::put($cacheKey, $itineraire->payload, now()->addDays(30));
        }

        // Fenêtre du déplacement (jours) → RSE.
        $dates = $postes->isNotEmpty() ? $postes->pluck('date') : $etapes->pluck('date');
        $dates = $dates->filter();
        if ($dates->isEmpty()) {
        ...
        $devis = [
            ...
            'original_distance_km' => round($itineraire->distanceKm, 2),
            'original_distance_km_charge' => round($itineraire->distanceKmCharge, 2),
            'original_distance_km_vide' => round($itineraire->distanceKmVide, 2),
            'original_duree_conduite_minutes' => $itineraire->dureeConduiteMinutes,
            'original_temps_attente_minutes' => $attenteMinutes,
            'calcul_payload' => [
    ...
    /**
     * Recalcule uniquement la partie RSE du payload à partir des postes existants.
     * Utilisé après modification manuelle des postes (ajout/suppression).
     */
    public function recalculerRsePourDevis(Devis $devis): void
    {
        $devis->loadMissing('demande.etapes.commune', 'postes');
        $postes = $devis->postes()->orderBy('date')->orderBy('ordre')->get();
        $dates = $postes->isNotEmpty() ? $postes->pluck('date') : $devis->demande->etapes->pluck('date');
        $dates = $dates->filter();
        if ($dates->isEmpty()) {
            return;
        }
        $dateDebut = Carbon::parse($dates->min());
        $dateFin = Carbon::parse($dates->max());
        $config = $this->rse->calculer($devis->duree_conduite_minutes, $dateDebut, $dateFin, $postes);

        $payload = $devis->calcul_payload ?? [];
        $payload['rse'] = $config->details;
